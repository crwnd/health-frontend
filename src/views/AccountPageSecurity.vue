    ...
    <div class="account-section">
      ...
      <div class="account-section-content">
        <ul class="account-section-content__list sessions-list">
          <li class="sessions-list__item" v-for="session in userObj.sessions" :data-id="session.id">
            <div class="sessions-list__item__info">
              <svg>
                <use :xlink:href="'/src/assets/icons.svg?005#codeslash-outline'" />
              </svg>
              <span>{{ session.infoString }}</span>
            </div>
            <button
              v-if="session.isActive && userObj.currentSessionID != session.id"
              @click="removeSession"
            >
              Delete
            </button>
            <span
              ...
              >Expired</span
            >
            <span v-if="userObj.currentSessionID == session.id">Current</span>
          </li>
        </ul>
      </div>
    </div>
